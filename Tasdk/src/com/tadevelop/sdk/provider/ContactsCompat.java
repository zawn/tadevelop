 ...
 *
 */
package com.tadevelop.sdk.provider;
...
public class ContactsCompat {
	...
	/**
	 * 返回手机通讯录列表,每个联系人对应一个HashMap,每个HashMap中键name对应{@link String}类型的联系人的姓名,键phones对应{@link List}类型的电话列表, <br>
	 * 每个电话为形如下的一维数组: <br>
	 * {number, type} <br>
	 * 其中number表示电话号码,type表示给号码的类型
	 * 
	 * @param mContext
	 * @return
	 */
	...
	public static List<HashMap<String, Object>> getContacts(Context mContext,String queryName) {
		return IMPL.getContacts(mContext,queryName);
	}
	...
	interface ContactsCompatImpl {
		...
		public List<HashMap<String, Object>> getContacts(Context mContext,String queryName);
	}
	...
	static class ContactsCompatImplBase implements ContactsCompatImpl {
			...
			try {
				...
				if (cCur != null && cCur.getCount() > 0) {
					...
					while (cCur.moveToNext()) {
						...
						Cursor peopleCur = cr.query(Contacts.Phones.CONTENT_URI, null, Contacts.Phones.PERSON_ID
								+ " = ?", new String[] { id }, null);
						...
						if (pCount > 0) {
							...
							while (peopleCur.moveToNext()) {
								phoneNumbers.add(new String[] { peopleCur.getString(numberColumn),
										peopleCur.getString(typeColumn) });
							}
						}
						peopleCur.close();
				...
				Log.e(TAG, "Did not have read contacts permission");
			} finally {
				if (cCur != null) {
					cCur.close();
				}
		...
		public List<HashMap<String, Object>> getContacts(Context mContext, String queryName) {
			...
			// 是否查询全部通讯录,如果姓名为空则是
			isQueryAll = queryName.equals("") ? true : false;
			if (isQueryAll) {
				selection = null;
			} else {
				selection = People.NAME + " like " + "'%" + queryName + "%'";
			}

			ContentResolver cr = mContext.getContentResolver();
			Cursor cCur = null;
			try {
				cCur = cr.query(People.CONTENT_URI, null, selection, null, null);
				if (cCur != null && cCur.getCount() > 0) {
					int idColumn = cCur.getColumnIndex(People._ID);
					int nameColumn = cCur.getColumnIndex(People.NAME);
					int displayNameColumn = cCur.getColumnIndex(People.DISPLAY_NAME);
					...
					while (cCur.moveToNext()) {
						...
						peopleCur.close();
						// Log.i(TAG, "name:" + name + ",displayName:" + displayName + "," +
						// Arrays.deepToString(phoneNumbers.toArray()));
	...
	static class ContactsCompatImplEclair implements ContactsCompatImpl {
			...
			try {
				...
				if (cur != null && cur.moveToFirst()) {
					...
					do {
						...
						if (numberCount > 0) {
							Cursor peopleCur = mContext.getContentResolver().query(
		...
		public List<HashMap<String, Object>> getContacts(Context mContext, String queryName) {
			...
			if (isQueryAll) {
				...
				selection = ContactsContract.Contacts.DISPLAY_NAME + " like " + "'%" + queryName + "%'";
			}
