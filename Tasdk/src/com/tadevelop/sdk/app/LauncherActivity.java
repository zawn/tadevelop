 ...
 * {@code
 *  <activity
 * 	 android:name="com.tadevelop.sdk.app.LauncherActivity"
 ...
 * @author Zawn
 */
final public class LauncherActivity extends Activity {
	...
	private void onHandleIntent(final Intent intent) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.onHandleIntent() TaskId:" + this.getTaskId());
		...
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.Target class is : " + ((cls == null) ? "null" : cls.getName()));
		final Intent i;
		if (cls == null) {
			if (DEBUG)
				Log.i(TAG, "LauncherActivity.Start the default activity : " + mDefaultLaunchInent.getComponent());
			...
			if (cls.isInstance(LauncherActivity.this)) {
				if ((Intent.FLAG_ACTIVITY_CLEAR_TOP & intent.getFlags()) == 0) {
					throw new RuntimeException(
							"LauncherActivity is the root of this task.  If you want to exit this task, add Intent.FLAG_ACTIVITY_CLEAR_TOP flag in intent.");
				}
				initiativeDestroy();
			} else {
	...
	protected void onCreate(Bundle savedInstanceState) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.onCreate() TaskId:" + this.getTaskId());
		super.onCreate(savedInstanceState);
		if (!isTaskRoot()) {
			Log.e(TAG, "LauncherActivity is not the root of this task. Please confirm the code correctly");
			finish();
		}
	...
	final protected void onDestroy() {
		...
		if (mInitiativeDestroy) {
			...
			if (DEBUG)
				Log.i(TAG, "LauncherActivity.java Terminate this instance");
	...
	final protected void onNewIntent(final Intent intent) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.onNewIntent()");
		mIsNewIntent = true;
		setIntent(intent);
		onHandleIntent(intent);
	}
	...
	final protected void onResume() {
		super.onResume();
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.onResume()");
		if (mIsNewIntent) {
	...
	final protected void onSaveInstanceState(Bundle outState) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.onSaveInstanceState()");
	...
	private void initiativeDestroy() {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.initiativeDestroy()");
	 ...
	 * {@link LauncherActivity} 以外的所有{@link Activity} ,并启动方法参数 <code>cls </code>指定的 {@link Activity}<br>
	 * <br>
	 * 注:如果cls指定的{@link Activity}是{@link LauncherActivity}本身,因为{@link LauncherActivity}是不可见的此<br>
	 * 时{@link LauncherActivity}将自我销毁,程序退出.
	 * 
	...
	public static void switchActivity(Activity activity, Class<?> cls) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.switchActivity(), cls = " + cls.getName());
	...
	public static void switchActivity(Activity activity, Class<?> cls, Bundle bundle) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.switchActivity(), cls = " + cls.getName() + ", bundle = "
	...
	public static void exitTask(Context context) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.exitTask()");
		Intent intent = new Intent(context, LauncherActivity.class);
		intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
		intent.putExtra(CURRENT_LAUNCH_ACTIVITY, LauncherActivity.class);
	...
	public static void reStart(Activity activity) {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.reStart()");
	...
	private Intent getDefaultLaunchIntent() {
		if (DEBUG)
			Log.i(TAG, "LauncherActivity.getDefaultLaunchIntent()");
		...
		if (activityName != null) {
			...
			throw new RuntimeException(
					"LauncherActivity did not find the default to launch Activity, make sure you provide the name of the class is correct and complete");
		}
		if (DEBUG) 
